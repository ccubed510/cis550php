...
<table>
	{
	for $belongsTo in /photodb/belongsTo 
	return 
	<row>
		<friendID>
			{
			$belongsTo/friendID/text()
			}
		</friendID>
		<circleID>
			{
			concat(/photodb/circle[circleID=$belongsTo/circleID]/ownerID/text(),$belongsTo/circleID/text())
			}
		</circleID>
	</row>
	}
	...
	<MapperMetaTag>
		...
		<MapperBlockPosition>
			<template name="xquery_body">
				<block path="table/flwr" x="93" y="62"/>
			</template>
		</MapperBlockPosition>
